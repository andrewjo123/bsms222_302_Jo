...
ChickWeight is tidy: each observation (a weight) is represented by one row. The chick from which this measurement came from is one the variables.

ChickWeight is not a tidy: we are missing the year column.
...
```{r}
# murders %>% select(state, region, rate) %>% filter(rate <= 0.71) rate X
```
