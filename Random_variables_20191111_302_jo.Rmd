...
We can use the function dbinom and pbinom to compute the probabilities exactly. For example, to compute Pr(S<0)  we note that: Pr(S<0)=Pr((S+n)/2<(0+n)/2) and we can use the pbinom to compute Pr(S≤0)
...
This is because the sd function R does not return the sd of the list, but rather uses a formula that estimates standard deviations of a population from a random sample  X1,…,XN  which, for reasons not discussed here, divide the sum of squares by the  N−1 .¯X=1NN∑i=1Xi,s=⎷1N−1N∑i=1(Xi−¯X)2
