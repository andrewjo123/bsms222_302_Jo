...
6. To see the math that explains why the answers to questions 3, 4, and 5 are the same, suppose we have a random variable with average  m  and standard error  s . Suppose we ask the probability of  X  being smaller or equal to  a . Remember that, by definition,  a  is  (a−m)/s  standard deviations  s  away from the average  
m. The probability is:

Pr(X≤a)
 

Now we subtract  μ  to both sides and then divide both sides by  σ : Pr(X−ms≤a−ms)
